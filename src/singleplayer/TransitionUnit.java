...
public class TransitionUnit extends GraphicalGameUnit{
	...
	public TransitionUnit(UnitState nextUnitState, BufferedImage message, GraphicalGameUnit nextUnit) {
	...
	public void handleKeyPressed(KeyEvent e) {
		...
		if (key == KeyEvent.VK_ENTER){
			if (nextUnit != null) {
				getNavigator().addGameUnit(nextUnit, nextUnitState);
			}
			System.out.println(nextUnitState);
			getNavigator().set(nextUnitState);
		}
	}
	...
	public void drawComponent(Graphics g) {
		...
		g.drawImage(message, 0, 0, message.getWidth(), message.getHeight(), null);
	}
